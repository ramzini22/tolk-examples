...
import { GasLogAndSave } from '../utils/gas-logger';
...
describe(numericFolder, () => {
    ...
    beforeAll(async () => {
        jwallet_code = await compile('jetton-wallet');
    ...
    it('[bench] minter admin should be able to mint jettons', async () => {
        ...
        const transactions = await blockchain.getTransactions(user2.address);

        expect(await user1JettonWallet.getJettonBalance()).toBe(initialJettonBalance);

        await user1JettonWallet.sendTransfer(
            user1.getSender(),
            toNano('0.1'), //tons
            sentAmount,
            user2JettonWallet.address,
            user2.address,
            new Cell(),
            forwardAmount,
            new Cell(),
        ...
        expect(await user1JettonWallet.getJettonBalance()).toBe(toNano('0'));
        ...
        // const JettonWallet = await userWallet(user1.address);
    ...
    //     let forwardAmount = toNano('0.05');
    //     const sendResult: SendMessageResult = await deployerJettonWallet.sendTransfer(
    //         deployer.getSender(),
    //         toNano('0.1'), //tons
    //         sentAmount,
    //         notDeployer.address,
    ...
    //         .storeUint(0, 1)
    //         .endCell();
    //     const sendResult: SendMessageResult = await blockchain.sendMessage(
    //         internal({
    //             from: notDeployer.address,
    //             to: deployerJettonWallet.address,
    ...
    //     const sendResult: SendMessageResult = await deployerJettonWallet.sendBurn(
    //         deployer.getSender(),
    ...
    //     let discoveryResult: SendMessageResult = await jettonMinter.sendDiscovery(
    //         deployer.getSender(),
    //         deployer.address,
    //         true,
    //     );
